function punchline()
    ...
    VARYING_THRUST = multiplier*j;

    for i=1:21
        TIMES_SPENT(i) = plane2(multiplier*j(i));
    end
    
    plot(VARYING_THRUST, TIMES_SPENT, 'o');
    ...
    % Modeling a Boeing 727
        ...
        [TIME_3, Y_2] = ode45(@thrust_on_2, [0, 2000], [23560, 7300, 110, -182], options);
        T = TIME_3;
        Y = Y_2;
        
        [TIME_2, Y_2] = ode45(@thrust_off, [T(end), 4000], [Y(end, 1), Y(end, 2), Y(end, 3), Y(end, 4)], options2);
        Y = [Y; Y_2];
        T = [T; TIME_2];
